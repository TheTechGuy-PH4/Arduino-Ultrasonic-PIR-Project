...
void rt_OneStep(void)
{
  ...
  Final_Project_Simulink_Model_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Final_Project_Simulink_Model_M, 0);
  ...
  Final_Project_Simulink_Model_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(Final_Project_Simulink_Model_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (Final_Project_Simulink_Model_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Final_Project_Simulink_Model_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Final_Project_Simulink_Model_M);
  ...
  /* Terminate model */
  Final_Project_Simulink_Model_terminate();
